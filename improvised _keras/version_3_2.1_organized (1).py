...
def backtest_model(stock_data, train_start, train_end, test_start, test_end, tickers):
    train_data = pd.concat([stock_data[ticker].loc[train_start:train_end] for ticker in tickers])
    test_data = pd.concat([stock_data[ticker].loc[test_start:test_end] for ticker in tickers])

    for ticker in tickers:
        train_data.loc[train_data['Ticker'] == ticker, 'Weekly_Return'] = train_data.loc[train_data['Ticker'] == ticker, 'Close'].pct_change(periods=5)
        test_data.loc[test_data['Ticker'] == ticker, 'Weekly_Return'] = test_data.loc[test_data['Ticker'] == ticker, 'Close'].pct_change(periods=5)

    # Identify best-performing stock each week in the training data
    train_weekly_best = train_data.groupby(['Week', 'Ticker'])['Weekly_Return'].mean().reset_index().groupby('Week')['Weekly_Return'].idxmax()
    train_best_tickers = train_data.loc[train_weekly_best, ['Week', 'Ticker']]

    # Merge to identify the best-performing stock each week
    train_data = pd.merge(train_data, train_best_tickers, on='Week', how='left')
    train_data['Target'] = (train_data['Ticker_x'] == train_data['Ticker_y']).astype(int)

    # Prepare features and target for training
    scaler = StandardScaler()
    features = ['RSI', 'SMA', 'EMA', 'Avg_Sentiment', 'RSI_Nifty', 'SMA_Nifty', 'EMA_Nifty']
    X_train = scaler.fit_transform(train_data[features].fillna(0))
    y_train = train_data['Target']

    # Prepare test data
    test_weekly_best = test_data.groupby(['Week', 'Ticker'])['Weekly_Return'].mean().reset_index().groupby('Week')['Weekly_Return'].idxmax()
    test_best_tickers = test_data.loc[test_weekly_best, ['Week', 'Ticker']]
    test_data = pd.merge(test_data, test_best_tickers, on='Week', how='left')
    test_data['Target'] = (test_data['Ticker_x'] == test_data['Ticker_y']).astype(int)
    X_test = scaler.transform(test_data[features].fillna(0))
...
# Assuming 'stock_data' contains data for all stocks from 2012 to 2023
train_start = '2012-01-01'
train_end = '2018-12-31'
test_start = '2019-01-01'
test_end = '2023-01-26'

backtest_accuracy = backtest_model(stock_data, train_start, train_end, test_start, test_end, stocks[:-1])  # Exclude '^NSEI' for backtesting
